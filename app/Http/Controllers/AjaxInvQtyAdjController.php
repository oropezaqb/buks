...
class AjaxInvQtyAdjController extends Controller
{
    ...
    {
        $quantities = array();
        $quantityPurchased = \DB::table('purchases')->where('product_id', $productId)
            ->whereDate('date', '<=', $adjDate)->sum('quantity');
        $quantitySold = \DB::table('sales')->where('product_id', $productId)
            ->whereDate('date', '<=', $adjDate)->sum('quantity');
